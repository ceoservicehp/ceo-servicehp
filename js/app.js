(function () {
  ...
  const state = {
    ...
    salaryMonth: localDate().slice(0, 7), salaryTab: "daily", parsedImport: null
  };
  ...
  function renderSalary() {
    ...
    return `
      ...
      <section class="salary-panel hidden" data-salary-panel="history"><section class="card"><h4>Riwayat gaji dan pengambilan</h4><div class="search-row"><input id="salarySearch" type="search" placeholder="Cari nama karyawan"><select id="salarySort"><option value="az">Nama A–Z</option><option value="za">Nama Z–A</option><option value="newest">Tanggal terbaru</option></select></div><div class="table-wrap"><table><thead><tr><th>Tanggal</th><th>Nama</th><th>Jenis</th><th>Status/Metode</th><th>Pokok</th><th>Tambahan</th><th>Potongan</th><th>Total</th><th>Catatan</th><th>Aksi</th></tr></thead><tbody id="salaryHistory">${salaryHistoryRows("", "az")}</tbody></table></div></section></section>`;
  }

  function salaryHistoryRows(query, sortMode) {
    const earned = state.salaries.map(row => ({ id: row.id, source: "salary", date: row.salary_date, employee_name: row.employee_name, type: "Gaji harian", status: String(row.attendance_status || (row.present ? "hadir" : "alpa")).replaceAll("_", " "), base: num(row.base_salary), additions: num(row.allowance) + num(row.overtime) + num(row.bonus), deduction: num(row.deduction), total: num(row.total), notes: row.notes || "" }));
    const taken = state.withdrawals.map(row => ({ id: row.id, source: "withdrawal", date: row.withdrawal_date, employee_name: row.employee_name, type: "Pengambilan gaji", status: String(row.payment_method || "cash").toUpperCase(), base: null, additions: null, deduction: null, total: -num(row.amount), notes: [row.reference_number ? `Ref: ${row.reference_number}` : "", row.notes || ""].filter(Boolean).join(" · ") }));
    const openings = state.openingBalances.map(row => ({ id: row.id, source: "opening-balance", date: row.effective_date, employee_name: row.employee_name, type: "Saldo awal", status: "Awal sistem", base: num(row.prior_salary), additions: num(row.prior_bonus), deduction: num(row.prior_withdrawn), total: num(row.prior_salary) + num(row.prior_bonus) - num(row.prior_withdrawn), notes: row.notes || "Catatan sebelum sistem" }));
    ...
    return rows.length ? rows.map(row => { const adminOnly = row.source === "opening-balance" ? " data-admin-action" : ""; return `<tr><td>${formatDate(row.date)}</td><td><strong>${escapeHtml(row.employee_name)}</strong></td><td>${row.type}</td><td><span class="pill">${escapeHtml(row.status)}</span></td><td>${row.base === null ? "-" : rupiah(row.base)}</td><td>${row.additions === null ? "-" : rupiah(row.additions)}</td><td>${row.deduction === null ? "-" : rupiah(row.deduction)}</td><td class="${row.total < 0 ? "negative" : "positive"}"><strong>${row.total < 0 ? "− " : ""}${rupiah(Math.abs(row.total))}</strong></td><td>${escapeHtml(row.notes || "-")}</td><td><div class="button-row"><button class="button edit small" data-action="edit-${row.source}" data-id="${row.id}"${adminOnly}>Edit</button><button class="button danger small" data-action="delete-${row.source}" data-id="${row.id}"${adminOnly}>Hapus</button></div></td></tr>`; }).join("") : '<tr><td colspan="10" class="empty">Data tidak ditemukan.</td></tr>';
  }
